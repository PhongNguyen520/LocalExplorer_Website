...
const CreateBusiness = () => {
  ...
  const handlePayment = async () => {
    if (!createdBusinessId || !form.pricingPlanId) return;
    try {
      const res = await payOSApi(form.pricingPlanId, createdBusinessId);
      const payUrl = res?.data?.data?.payUrl || res?.data?.data?.url;
      if (payUrl) {
    ...
    <div className="p-6 flex justify-center">
      ...
      <Modal show={showSuccessModal} onClose={() => setShowSuccessModal(false)}>
        <div className="flex flex-col items-center gap-4">
          ...
          <button
            className="mt-2 px-6 py-2 bg-pink-600 text-white rounded-lg font-semibold hover:bg-pink-700 transition"
            onClick={handlePayment}
            disabled={!createdBusinessId || !form.pricingPlanId}
          >
            Thanh toán
          </button>
        </div>
